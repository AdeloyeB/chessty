...
// Black pieces: dark grey stroke (#888888), no fill
const PIECE_SVGS: Record<Color, Record<PieceType, (props: { size: number }) => React.JSX.Element>> = {
  ...
  b: {
    k: ({ size }) => (
      <svg viewBox="0 0 45 45" width={size} height={size}>
        <g fill="none" stroke="#888888" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M22.5 11.63V6M20 8h5"/>
          <path d="M22.5 25s4.5-7.5 3-10.5c0 0-1-2.5-3-2.5s-3 2.5-3 2.5c-1.5 3 3 10.5 3 10.5"/>
          <path d="M11.5 37c5.5 3.5 15.5 3.5 21 0v-7s9-4.5 6-10.5c-4-6.5-13.5-3.5-16 4V27v-3.5c-3.5-7.5-13-10.5-16-4-3 6 5 10 5 10V37z"/>
          <path d="M11.5 30c5.5-3 15.5-3 21 0m-21 3.5c5.5-3 15.5-3 21 0m-21 3.5c5.5-3 15.5-3 21 0"/>
    ...
    q: ({ size }) => (
      <svg viewBox="0 0 45 45" width={size} height={size}>
        <g fill="none" stroke="#888888" strokeWidth="2" strokeLinejoin="round">
          <circle cx="6" cy="12" r="2.5"/>
          <circle cx="14" cy="9" r="2.5"/>
          <circle cx="22.5" cy="8" r="2.5"/>
          <circle cx="31" cy="9" r="2.5"/>
    ...
    n: ({ size }) => (
      <svg viewBox="0 0 45 45" width={size} height={size}>
        <g fill="none" stroke="#888888" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M22 10c10.5 1 16.5 8 16 29H15c0-9 10-6.5 8-21"/>
          <path d="M24 18c.38 2.91-5.55 7.37-8 9-3 2-2.82 4.34-5 4-1.042-.94 1.41-3.04 0-3-1 0 .19 1.23-1 2-1 0-4.003 1-4-4 0-2 6-12 6-12s1.89-1.9 2-3.5c-.73-.994-.5-2-.5-3 1-1 3 2.5 3 2.5h2s.78-1.992 2.5-3c1 0 1 3 1 3"/>
          <circle cx="9" cy="25.5" r="0.5" fill="#888888"/>
          <ellipse cx="14.7" cy="15.8" rx="0.5" ry="1.5" transform="rotate(30 14.7 15.8)" fill="#888888"/>
        </g>
      </svg>
    ),
    p: ({ size }) => (
      <svg viewBox="0 0 45 45" width={size} height={size}>
        <path d="M22.5 9c-2.21 0-4 1.79-4 4 0 .89.29 1.71.78 2.38C17.33 16.5 16 18.59 16 21c0 2.03.94 3.84 2.41 5.03-3 1.06-7.41 5.55-7.41 13.47h23c0-7.92-4.41-12.41-7.41-13.47 1.47-1.19 2.41-3 2.41-5.03 0-2.41-1.33-4.5-3.28-5.62.49-.67.78-1.49.78-2.38 0-2.21-1.79-4-4-4z" fill="none" stroke="#888888" strokeWidth="2" strokeLinecap="round"/>
      </svg>
    ),
  },
};
...
export function ChessBoard({
  ...
  const renderSquare = (row: number, col: number) => {
    ...
    // Connect 4 on paper style: pure black cells with white grid lines
    // Border on right and bottom edges only (grid shares borders)
    const isRightEdge = col === 7;
    const isBottomEdge = row === 7;

    const coordFontSize = Math.max(10, Math.min(cellWidth, cellHeight) * 0.18);

    return (
      <div
        key={square}
        className={`relative bg-black ${!isRightEdge ? 'border-r' : ''} ${!isBottomEdge ? 'border-b' : ''} border-white/15`}
        onClick={() => handleSquareClick(row, col)}
      >
        {/* Last move highlight - subtle white glow */}
        {isLastMoveSquare && (
        ...
        {isSelected && (
          <div className="absolute inset-0 bg-white/10 ring-1 ring-inset ring-white/30" />
        )}

        {/* Check highlight */}
        {isCheckSquare && (
          <div className="absolute inset-0 bg-red-500/15 ring-1 ring-inset ring-red-400/40" />
        )}
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            {piece ? (
              ...
              <div
                ...
                style={{ width: Math.min(cellWidth, cellHeight) * 0.25, height: Math.min(cellWidth, cellHeight) * 0.25 }}
              />
            )}
          </div>
        )}
        ...
        {col === 0 && (
          <span
            className="absolute left-[3px] top-[2px] font-mono pointer-events-none select-none text-white/20"
        ...
        {row === 7 && (
          <span
            className="absolute right-[3px] bottom-[2px] font-mono pointer-events-none select-none text-white/20"
            style={{ fontSize: coordFontSize }}
          >
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full select-none bg-black border border-white/15 relative"
    >
