/**
 * Arbiter Overwatch Event Handler
 * ================================
 *
 * Connects the Arbiter Overwatch system to the game event flow.
 * Listens for anti-cheat events and creates overwatch cases for high-suspicion games.
 *
 * HOW IT WORKS:
 * 1. Listens for anticheat:review_required events (fired when suspicion > 95%)
 * 2. Creates an overwatch case for the flagged game
 * 3. Assigns eligible arbiters to review the case
 ...
 * arbiter accuracy. These are mixed in with real cases.
 */
...
/** Minimum suspicion score to create an overwatch case */
const OVERWATCH_CASE_THRESHOLD = 0.95;
...
/**
 * Register Arbiter Overwatch event handlers.
 *
 ...
 */
export function registerOverwatchHandlers(events: GameEventEmitter): void {
  // ---------------------------------------------------------------------------
  // Review Required Handler
  // ---------------------------------------------------------------------------
  //
  // When the anti-cheat system flags a game for human review (suspicion > 95%),
  // we create an overwatch case for community review.

  events.on(
    'anticheat:review_required',
    async (payload) => {
      try {
        // Only create overwatch cases for high suspicion scores
        // The threshold should match SUSPICION_THRESHOLDS.REVIEW from anticheat
        if (payload.score < OVERWATCH_CASE_THRESHOLD * 100) {
          console.log(
            `[Overwatch] Skipping case creation for game ${payload.gameId}: ` +
            `score ${payload.score} below threshold ${OVERWATCH_CASE_THRESHOLD * 100}`
          );
          return;
        }
        ...
        // Create the overwatch case
        const overwatchCase = await createCase({
        ...
        console.log(
          `[Overwatch] Created case ${overwatchCase.id} for game ${payload.gameId} ` +
          `(player: ${payload.playerId}, priority: ${priority})`
        );
      } catch (error) {
        // Log but don't throw - overwatch case creation shouldn't break the event chain
        console.error(
          `[Overwatch] Error creating case for game ${payload.gameId}:`,
    ...
    },
    {
      ...
      label: 'overwatch:review_required',
    }
  );
  ...
  // create overwatch cases. These are monitored but not yet ready for community review.

  events.on(
    'anticheat:game_flagged',
    (payload) => {
      // Just log for now - we don't create overwatch cases for lower thresholds
      // These games are queued for deep analysis first
      console.log(
        `[Overwatch] Game ${payload.gameId} flagged for monitoring ` +
    ...
    },
    {
      priority: 95,
      nonBlocking: true,
      label: 'overwatch:game_flagged',
    }
  );

  console.log('[Overwatch] Event handlers registered');
}
...
/**
 * Start periodic tasks for the Arbiter Overwatch system.
 *
 ...
 */
export function startOverwatchPeriodicTasks(): void {
  // Check for cases to resolve every 5 minutes
  setInterval(async () => {
    try {
      const resolved = await checkAndResolveCases();
      if (resolved > 0) {
        console.log(`[Overwatch] Periodic check resolved ${resolved} cases`);
      }
    } catch (error) {
      console.error('[Overwatch] Error in resolution check:', error);
    }
  ...
  setInterval(async () => {
    try {
      ...
      if (expired > 0) {
        console.log(`[Overwatch] Expired ${expired} pending assignments`);
      }
    } catch (error) {
      console.error('[Overwatch] Error in expiration check:', error);
    }
  }, EXPIRATION_CHECK_INTERVAL);

  console.log('[Overwatch] Periodic tasks started');
}
